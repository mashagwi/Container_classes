...
TEST_F(VectorTest, MaxSizeMethod) {
    EXPECT_EQ(vec_init_list_.max_size(), std::numeric_limits<typename s21::vector<int>::size_type>::max());
}
...
TEST_F(VectorTest, InsertOutOfRange) {
    ...
    EXPECT_THROW(vec_init_list_.insert(vec_init_list_.begin() + 8, 42), std::out_of_range);
}
