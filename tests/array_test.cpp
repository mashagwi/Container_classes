...
class ArrayTest : public ::testing::Test {
 protected:
  ...
  s21::array<std::string, 1> arr2_{"world"};
};
...
TEST_F(ArrayTest, cbegin_cend) {
  auto it_begin = arr0_.cbegin();
  for (std::size_t i = 0; i < arr0_.size(); ++i) ASSERT_EQ(*it_begin++, i + 1);

  auto it_end = arr0_.cend();
  ASSERT_EQ(it_end, arr0_.end());
}
...
TEST_F(ArrayTest, swap2) {
  s21::array<std::string, 1> arr1{"space"};
  std::array<std::string, 1> arr2{"peace"};
  std::array<std::string, 1> arr3{"space"};
